        ...
        subgraph "Content"
            ...
            subgraph "Main メインエリア"
                ...
                subgraph "ChatArea チャットエリア スクロール可能"
                    ...
                    AnalyzeBtn["AIに分析させるボタン<br>クリックでオンデマンド分析実行"]
                    AnalysisArea["AI分析コメント表示エリア<br>分析実行後に表示"]
                end
...
| AIに分析させるボタン | クエリ結果の下に表示。クリックでオンデマンドAI分析を実行 | 結果あり・分析未実施・ストリーミング中でない場合のみ表示 |
| 大量結果警告 | 100行以上の結果に対して警告ダイアログを表示 | 「分析する」「キャンセル」ボタン付き。トークン消費の注意喚起 |
| AI分析コメント | 分析実行後にストリーミング表示される分析コメント | 💡アイコン付き。青い左ボーダーで区別 |
