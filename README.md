...
v1.1では、GraphQL APIへの対応を追加しました。既存のMySQL/PostgreSQL接続に加え、GraphQL Introspectionベースのクエリ自動生成、結果の可視化、エラーハンドリングに対応し、データソースの柔軟性を大幅に向上させました。
...
### v1.1で追加した機能（GraphQL対応）

23. **GraphQL接続先管理**: 接続先登録フォームのDB種別に「GraphQL」を追加。GraphQL選択時はエンドポイントURL入力に切り替え。認証なし
24. **GraphQLクエリ自動生成**: LLMがGraphQLスキーマに基づいてGraphQLクエリ（Queryのみ、Mutation禁止）を自動生成
25. **GraphQLスキーマ自動取得**: Introspection Query（`__schema`）でType/Field情報を自動取得しLLMコンテキストに渡す
26. **GraphQLスキーマキャッシュ**: DB接続と同様、GraphQL選択時にIntrospectionでスキーマを取得しキャッシュ
27. **GraphQL接続テスト**: Introspection Queryが正常に返ればテスト成功と判定
28. **GraphQL結果のフラット化**: LLMに対してフラットなデータ構造を返すクエリ生成を指示。ネストしたJSONもテーブル・グラフで可視化可能に
29. **GraphQL結果可視化**: GraphQLレスポンスもDB接続と同じく棒グラフ・折れ線・円グラフ・テーブルで表示
30. **GraphQL会話分離**: GraphQL接続先ごとに会話履歴が分離される（DB接続先と同様）
31. **GraphQLエラーハンドリング**: GraphQLのerrors配列でのエラーにも「質問を変えてみてください」等のガイド付きエラー表示
...
| テスト実装フェーズ | $0.00 | 40分 | 0秒 | 0秒 | 0 / 0 |
| 成果まとめフェーズ | $0.00 | 4分 | 0秒 | 0秒 | 0 / 0 |
| 完了 | $0.00 | 23.9時間 | 0秒 | 0秒 | 0 / 0 |
| 要件確認フェーズ | $0.00 | 1.2時間 | 0秒 | 0秒 | 0 / 0 |
| 実装フェーズ | $0.00 | 58分 | 0秒 | 0秒 | 0 / 0 |
| テスト設計フェーズ | $0.00 | 6分 | 0秒 | 0秒 | 0 / 0 |
| テスト実装フェーズ | $0.00 | 36分 | 0秒 | 0秒 | 0 / 0 |
| 成果まとめフェーズ | $0.00 | 4分 | 0秒 | 0秒 | 0 / 0 |
